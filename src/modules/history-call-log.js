...
import { START_CHECK_CALL_LOG, checkCallLogOnStartKey } from '../common/common.js'
import { getRcCallLogs } from '../common/apis.js'
import { findMatchCallLog } from './match-log.js'
import copy from 'json-deep-copy'
import { Button, Spin, notification, message } from 'antd'
import LogItem from './history-call-log-item.js'
import { syncRcLog } from './log-event.js'
import './call-log-check.styl'
import { CloseCircleOutlined } from '@ant-design/icons'
import cachedSearch, { searchPhone } from './search.js'
...
export default class HistoryCallLogCheck extends Component {
  ...
  handleSubmit = async () => {
    ...
    const { callLogsSelected, callLogs } = this.state
    const r = await syncRcLog(callLogsSelected)
    if (r && r.length) {
      const successOnes = r.filter(s => {
        return s.result && !s.indb && !s.error
      })
      if (successOnes.length) {
        notification.success({
          message: `Created ${successOnes.length} logs`
        })
      }
    }
    this.setState({
