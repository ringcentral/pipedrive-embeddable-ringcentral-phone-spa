...
export async function searchByPersonId (personId) {
  console.debug('window.rc.syncTimestampDeal', window.rc.syncTimestampDeal)
  if (!window.rc.syncTimestampDeal) {
    await getAllDeals()
  }
  await initJsStore()
...
function getDeals (token, start) {
  let url = `${host}/api/v1/pipelines/1/deals?limit=500&start=${start}&get_summary=0&totals_convert_currency=default_currency&session_token=${token}&strict_mode=true&status=open`
